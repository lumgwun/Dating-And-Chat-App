...
import static com.lahoriagency.cikolive.NewPackage.TimeUtils.getDate;

public class ChatAdapterNew extends QBMessagesAdapter<QBChatMessage> implements StickyRecyclerHeadersAdapter<RecyclerView.ViewHolder> {
    private static final String TAG = ChatAdapterNew.class.getSimpleName();
    private final QBChatDialog chatDialog;
    private PaginationHistoryListener paginationListener;
    private int previousGetCount = 0;

    public ChatAdapterNew(Context context, QBChatDialog chatDialog, List<QBChatMessage> chatMessages) {
    ...
    public void onBindHeaderViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        QBChatMessage chatMessage = getItem(position);
        dateTextView.setText(getDate(chatMessage.getDateSent() * 1000));
