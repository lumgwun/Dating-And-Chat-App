...
public class UiUtils {
    ...
    public static Drawable getColoredCircleDrawable(@ColorInt int color) {
        GradientDrawable drawable = (GradientDrawable) ResourceUtils.getDrawable(R.drawable.shape_progressbar_circle);
    ...
    public static int getCircleColor(@IntRange(from = RANDOM_COLOR_START_RANGE, to = RANDOM_COLOR_END_RANGE)
                                             int colorPosition) {
        @SuppressLint("DefaultLocale") String colorIdName = String.format("random_color_%d", colorPosition + 1);
